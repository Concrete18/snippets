...
def sim_matcher(target_str, string_list, max_similarity=0.8, debug=False):
    ...
    for string in string_list:
        if string.lower() == target_str.lower():
            return string
        match_perc = SequenceMatcher(None, target_str.lower(), string.lower()).ratio()
...
def lev_distance(word1: str, word2: str, lower=True) -> int:
    """
    ...
    for i in range(len(word1) - 1, -1, -1):
        for j in range(len(word2) - 1, -1, -1):
            if word1[i] == word2[j]:
                ...
                min_change = min(cache[i + 1][j], cache[i][j + 1], cache[i + 1][j + 1])
